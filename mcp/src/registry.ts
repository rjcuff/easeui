// Thin typed client over the live easeUI registry endpoints. The MCP server owns
// no data: it reads easeui.dev/r/* at runtime so new components appear without a
...
function base(env: { REGISTRY_URL?: string }) {
  return (env.REGISTRY_URL ?? "https://easeui.dev").replace(/\/$/, "");
}
...
/** The shadcn install command for a component, using the public @easeui namespace. */
export function installCommand(slug: string, pm: PackageManager) {
  return `${PM_PREFIX[pm]} shadcn add @easeui/${slug}`;
}
